...
* adding [system extensions] "../configuration/system-extensions" >}})
* updating [kernel command line arguments] "../../reference/kernel" >}})
* using custom `META` contents, e.g. for [metal network configuration] "../../advanced/metal-network-configuration" >}})
* generating [SecureBoot] "../install/bare-metal-platforms/secureboot" >}}) images signed with a custom key
...
Now the `aws-amd64.raw.xz` file contains the customized Talos AWS disk image which can be uploaded as an AMI to the [AWS] "../install/cloud-platforms/aws" >}}).
...
The `imager` container image can be checked by [verifying its signature] "../../advanced/verifying-images" >}}).
...
* `iso` builds a Talos ISO image (see [ISO] "../install/bare-metal-platforms/iso" >}}))
* `secureboot-iso` builds a Talos ISO image with SecureBoot (see [SecureBoot] "../install/bare-metal-platforms/secureboot" >}}))
...
* `secureboot-installer` builds an installer container image with SecureBoot (see [SecureBoot] "../install/bare-metal-platforms/secureboot" >}}))
...
* `--image-cache` allows to use a [local image cache] "../configuration/image-cache" >}})
...
Now the `_out/aws-amd64.raw.xz` contains the customized Talos AWS disk image which can be uploaded as an AMI to the [AWS] "../install/cloud-platforms/aws" >}}).
