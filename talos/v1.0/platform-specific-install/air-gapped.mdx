...
We will use the [QEMU] "../talos-guides/install/local-platforms/qemu" >}}) provisioner available in `talosctl` to create a local cluster, but the same approach could be used to deploy Talos in bigger air-gapped networks.
...
- Requirements for the Talos [QEMU] "../talos-guides/install/local-platforms/qemu" >}}) cluster
...
Please see [pull-through cache guide] "../talos-guides/configuration/pull-through-cache" >}}) for an example using Harbor container registry with Talos.
